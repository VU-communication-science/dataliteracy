...
[^1]: It is also possible to use ANOVA with non-experimental (i.e. observational) data.
    ...
    For observational data it is more common to use [regression analysis](18-linear-regression.qmd), because this provides more flexibility to control for confounding variables.
...
#### 5.5.1.1.2. Repeated measures ANOVA
...
Underneath the hood, ANOVA is a linear model just like [regression analysis](18-linear-regression.qmd).
...
```{r how-to-use-it-1, message=F}
library(afex)
library(emmeans)
library(sjPlot)
```

For our examples we'll use our [Practice data](07-data-frames.qmd#practice-data).
...
```{r one-way-anova-1, message=F}
m = aov_car(trust_t2 ~ experiment_group + Error(id), data=d)
m
```

```{r one-way-anova-2, echo=F, results=F}
...
```{r one-way-anova-5, fig.height=3}
plot_model(m$lm, type='pred', terms='experiment_group')
```
:::

#### 5.5.2.1.1. *Experimental* versus *observed* factors
...
```{r experimental-versus-observed-factors-1, message=F}
...
```{r two-way-anova-2, fig.height=3}
...
```{r two-way-anova-4, results=F, message=F}
...
```{r two-way-anova-5, results=F, message=F}
...
## 5.5.3. Conditions and assumptions
...
For the other assumptions, which are more general to linear models, we refer to the [regression tutorial](18-linear-regression.qmd).
If you have not yet learned about regression, you can ignore the details of these other assumptions for now.

### 5.5.3.1. Homogeneity of variance
...
```{r homogeneity-of-variance-1, message=F}
library(car)
...
```{r homogeneity-of-variance-2, message=F}
leveneTest(trust_t2 ~ experiment_group*np_subscription, data=d)
```

#### 5.5.3.1.1. What if the variance is not equal?
...
```{r what-if-the-variance-is-not-equal-1, message=F}
vars = d |>
...
```{r what-if-the-variance-is-not-equal-2, echo=F, results=F}
