...
### 5.6.1.1. When do we use ANCOVA?

ANCOVA is useful for two reasons:

#### 5.6.1.1.1. 1. Controlling for confounding variables
...
##### 5.6.1.1.1.1. 2. Reducing unexplained variance
...
#### 5.6.1.1.2. What happens to variance when adding a covariate?
...
```{r how-to-use-it-1, echo = F, warning = F}
set.seed(123)
...
```{r one-way-ancova-1, echo=F, message = F}
...
```{r one-way-ancova-2, warning=F, message=F}
...
```{r un-adjusted-means-2, warning = F, message = F}
model_anova <- aov_car(
...
```{r un-adjusted-means-3, echo = F, message=FALSE}
library(patchwork)
...
#### 5.6.2.2.1. Independence of the covariate
...
```{r independence-of-the-covariate-1, message= F}
cov_indep <- aov_car(
...
```{r independence-of-the-covariate-2, echo=FALSE}
...
#### 5.6.2.2.2. Homogeneity of regression slopes
...
```{r homogeneity-of-regression-slopes-1, warning=F, message = F}
...
```{r homogeneity-of-regression-slopes-2, echo=F, message = F}
ggplot(robot_data, aes(familiarity, comfort, color = robot_design)) +
  geom_point(alpha = .5) +
  geom_smooth(method = "lm", se = FALSE) +
  theme_minimal()
...
```{r two-way-ancova-2, warning=F, message=F}
...
```{r comparing-models-1, message=F}
# Two-way ANOVA
