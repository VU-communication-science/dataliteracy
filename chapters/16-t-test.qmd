...
```{r what-is-a-t-test-1, echo=F, warning=F, fig.height=4}
...
#### 5.2.1.1.1. Independent samples t-test
...
#### 5.2.1.1.2. Paired samples t-test
...
```{r how-to-use-1, message=F}
library(car)
```

For the examples we'll use our [practice dataset](07-data-frames.qmd#practice-data). 
...
```{r one-sample-t-test-2}
t.test(news_consumption ~ 1, mu = 14, data=d)
```

```{r one-sample-t-test-3, echo=F, results=F}
...
### 5.2.2.2. Independent samples t-test
...
```{r independent-samples-t-test-1}
d |>
...
```{r independent-samples-t-test-2, warning=F}
...
```{r independent-samples-t-test-3}
t.test(news_consumption ~ np_subscription, data = d, var.equal = FALSE)
```

```{r independent-samples-t-test-4, echo=F, results=F}
...
Note that in reality, when you have both repeated measures and multiple groups, you might want to use a mixed design [ANOVA](19-anova.qmd) instead.

```{r paired-samples-t-test-1}
d_positive <- filter(d, experiment_group == "positive")
```

First, let's look at the means and standard deviations for each time point.

```{r paired-samples-t-test-2}
d_positive |>
...
```{r paired-samples-t-test-3}
t.test(Pair(trust_t1, trust_t2) ~ 1, data = d_positive)
```

```{r paired-samples-t-test-4, echo=F, results=F}
...
### 5.2.2.4. Two-sided vs one-sided t-test
...
```{r two-sided-vs-one-sided-t-test-1, echo=F, fig.height=4}
# Load the required libraries
library(ggplot2)
library(gridExtra)
...
#### 5.2.2.4.2. How to use a one-sided t-test
...
```{r how-to-use-a-one-sided-t-test-1}
...
```{r how-to-use-a-one-sided-t-test-2}
t.test(news_consumption ~ 1, mu = 14, data = d, alternative = "less")
```

#### 5.2.2.4.3. one-sided test in independent and paired samples t-test
