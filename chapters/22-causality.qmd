...
## 6.2.1. Testing causal relations
...
## 6.2.2. Why correlation does not imply causation
...
[Correlation](17-correlation.qmd), on the other hand, only indicates an association or relationship between two variables.
...
```{r visualizing-causation-1, echo=F, message=F, fig.height=4, warning=F}
library(ggdag)
library(gridExtra)
library(tidyverse)
...
## 6.2.3. How to (better) establish causation
...
### 6.2.3.1. Theoretical foundation
...
### 6.2.3.2. If possible, conduct an experiment
...
### 6.2.3.3. For observational data, control for confounders
...
Using techniques like [multiple regression](18-linear-regression.qmd), we can then measure the effect of the independent variable on the dependent variable while holding the confounding variable constant.
