...
### 3.1.1.1. Installing the tidyverse {#installing-the-tidyverse}
...
```{r installing-the-tidyverse-1, eval=F}
install.packages('tidyverse')
```

Then in every script that you use the package, you open it with `library`:

```{r installing-the-tidyverse-2, message=FALSE}
library(tidyverse)
```
...
```{r installing-the-tidyverse-3, eval=F}
?dplyr::filter()
?stats::filter()
```
...
### 3.1.1.2. Data management with the tidyverse {#data-management-with-the-tidyverse}
...
```{r data-management-with-the-tidyverse-1}
...
```{r data-management-with-the-tidyverse-2}
d
```
...
### 3.1.2.1. Reading and writing data {#importing-data}
...
#### 3.1.2.1.2. Importing data from a URL
...
```{r importing-data-from-a-url-1, message=FALSE}
library(tidyverse)
...
```{r importing-data-from-a-url-2, results=FALSE}
d
```
...
```{r importing-data-from-a-url-3, eval=F}
View(d)
```
...
#### 3.1.2.1.3. Writing data to a CSV file on your computer

You can use the `write_csv` function to write a tibble to a CSV file on your computer.
If you just provide a file name, it will be saved in your current [working directory](05-projects.qmd#working-directory).

```{r writing-data-to-a-csv-file-on-your-computer-1, eval=FALSE}
write_csv(d, "practice_data.csv")
```


#### 3.1.2.1.4. Reading data from a CSV file on your computer

Now let's read this file back into R.
Since the file is in your working directory, you can just specify the file name:

```{r reading-data-from-a-csv-file-on-your-computer-1, eval=FALSE}
...
#### 3.1.2.1.5. Pitfall 1: Corrupting the file by opening it in Excel
...
#### 3.1.2.1.6. Pitfall 2: Different flavours of CSV files
...
#### 3.1.2.1.7. Reading other file formats, like Excel and SPSS
...
### 3.1.3.1. Selecting columns with `select`
...
```{r selecting-columns-with-select-1}
colnames(d)
```

#### 3.1.3.1.1. Selecting specific columns
...
##### 3.1.3.1.1.1. Selecting a range of columns
...
```{r selecting-a-range-of-columns-1, results='hide'}
...
##### 3.1.3.1.1.2. Selecting and renaming columns
...
```{r selecting-and-renaming-columns-1, results='hide'}
...
##### 3.1.3.1.1.3. Selecting columns that have spaces in the name  
...
```{r selecting-columns-that-have-spaces-in-the-name-1, results='hide'}
select(d, news_consumption = `news consumption`)
```

##### 3.1.3.1.1.4. Dropping columns
...
```{r dropping-columns-1, results='hide'}
...
### 3.1.3.2. Renaming columns with `rename`
...
```{r renaming-columns-with-rename-1, results='hide'}
...
## 3.1.4. Filter and arrange

### 3.1.4.1. Subsetting rows with filter() {#filter} 
...
```{r subsetting-rows-with-filter-1}
...
```{r subsetting-rows-with-filter-2, output=FALSE}
...
```{r subsetting-rows-with-filter-3, output=FALSE}
...
```{r subsetting-rows-with-filter-4, output=FALSE}
filter(d, !experiment_group %in% c('positive', 'negative'))
```
...
#### 3.1.4.1.1. The condition is a logical expression
...
##### 3.1.4.1.1.1. Comparison operators
...
```{r using-equations-1, output=FALSE}
filter(d, abs(trust_t2 - trust_t1) > 2)
```

##### 3.1.4.1.1.4. Parentheses
...
```{r filtering-out-missing-values-1}
filter(d, is.na(age))     ## rows where age IS missing
```
```{r filtering-out-missing-values-2, results='hide'}
...
```{r sorting-rows-with-arrange-2, output=FALSE}
arrange(d, -age)
```
...
```{r sorting-rows-with-arrange-3, output=FALSE}
arrange(d, experiment_group, -age)
```

## 3.1.5. Mutate and recode

### 3.1.5.1. Creating and modifying columns with mutate() {#mutate}
...
#### 3.1.5.1.1. Creating new variables

To create a new variable you use the following syntax:

```{r creating-new-variables-1, eval=F}
...
#### 3.1.5.1.2. Mutate existing variables
...
```{r mutate-existing-variables-2, results=T}
hist(d$trust_change)
```


### 3.1.5.2. Recoding variables {#recode}
...
#### 3.1.5.2.1. Recode with `case_match`
...
```{r recode-with-case-match-2}
unique(d$experiment_group)
```

#### 3.1.5.2.2. More flexible recoding with `case_when`
...
```{r more-flexible-recoding-with-case-when-1}
...
#### 3.1.5.2.3. Binary cases with `if_else`
...
```{r binary-cases-with-if-else-1, eval=F}
...
## 3.1.6. Using the pipe syntax

### 3.1.6.1. Working with Pipes

In the [Functions](04-working-with-r.qmd#using-the-pipe-syntax) tutorial we already mentioned that R also has a `pipe` syntax. 
...
#### 3.1.6.1.1. Using pipes with the tidyverse
...
```{r using-pipes-with-the-tidyverse-1, message=F}
library(tidyverse)
...
```{r using-pipes-with-the-tidyverse-2, message=F}
practice_data = read_csv("https://data.dataliteracy.cc/simulated/practice_data.csv") %>%
