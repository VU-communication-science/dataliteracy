...
## 3.2.1. Inspect and clean your data
...
### 3.2.1.1. First, get to know your data
...
```{r first-get-to-know-your-data-1, results="hide"}
d
```
...
```{r first-get-to-know-your-data-2, eval=FALSE}
View(d)
```
```{r first-get-to-know-your-data-3, echo=FALSE}
d
```


### 3.2.1.2. Viewing a summary of every column
...
```{r viewing-a-summary-of-every-column-1, results="hide"}
summary(d)
```
...
```{r viewing-a-summary-of-every-column-2, output=FALSE, message=FALSE}
library(summarytools)
dfSummary(d)
```
...
```{r viewing-a-summary-of-every-column-3, eval=FALSE}
dfSummary(d) |> view()
```

```{r viewing-a-summary-of-every-column-4, echo=FALSE}
print(dfSummary(d), 
      method='render', 
      max.tbl.height = 500)
```
...
```{r viewing-a-summary-of-every-column-5, eval=FALSE}
...
#### 3.2.1.2.1. What to look for in the summary
...
```{r what-to-look-for-in-the-summary-1}
unique(d$age)
```
...
#### 3.2.2.2.1. Option 1: Subtract the birth year from the year of the study
...
```{r option-1-subtract-the-birth-year-from-the-year-of-the-study-1}
...
#### 3.2.2.2.2. Option 2: Recode specific values
...
```{r option-2-recode-specific-values-1}
d_clean = mutate(d_clean, age = recode(age, `1987` = 37, `1970` = 51, `1967` = 54))
```


### 3.2.2.3. Removing missing values
...
```{r removing-missing-values-2}
d_clean <- filter(d_clean, !is.na(age))
```

### 3.2.2.4. Filtering rows
...
```{r check-again-1, eval=FALSE}
dfSummary(d_clean) |> view()
```

```{r check-again-2, echo=FALSE}
