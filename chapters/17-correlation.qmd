...
## 5.3.1. What is the correlation test?
...
In this tutorial we focus on conducting the test. For more details on the concept of correlation and how to interpret the correlation coefficient $r$, see the [covariance and correlation](17-correlation.qmd) section.
...
For more detail on this, see the [causality](22-causality.qmd) section.
...
### 5.3.2.1. Between two variables: cor.test
...
```{r between-two-variables-cor-test-1}
cor.test(d$news_consumption, d$trust_t1)
```

```{r between-two-variables-cor-test-2, echo=F, results=F}
...
```{r between-two-variables-cor-test-3, results='hide'}
cor.test(~ news_consumption + trust_t1, data = d)
```
:::

### 5.3.2.2. Between more variables: tab_corr
...
```{r between-more-variables-tab-corr-1, message=F}
library(sjPlot)
d |>
...
## 5.3.3. Conditions and assumptions
...
### 5.3.3.1. Spearman's $\rho$ and Kendall's $\tau$
...
```{r spearman-s-rho-and-kendall-s-tau-1, results='hide', warning=F, message=F}
