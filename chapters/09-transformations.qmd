...
* **Correlation analysis**: If you calculate the covariance of two standardized variables, you get the [correlation coefficient](17-correlation.qmd), which is a useful measure of the strength and direction of the relationship between two variables.
* **Multiple regression**: Standardization can help you compare the strength of the effects of different independent variables in a [multiple regression model](18-linear-regression.qmd).


## 3.3.2. How to standardize variables

Let's first create a simple numeric variable `x` with some values, and calculate its mean and standard deviation.

```{r how-to-standardize-variables-1, message=F}
x <- c(4, 9, 6, 3, 9, 10)
...
```{r how-to-standardize-variables-2}
z <- (x - mean(x)) / sd(x)
...
```{r how-to-standardize-variables-3}
cor(x, z)
```


### 3.3.2.1. Alternative: use the `scale()` function

There is also a built-in function in R that standardizes variables for you.
This function is called `scale()`.

```{r alternative-use-the-scale-function-1}
scale(x)
```
...
```{r alternative-use-the-scale-function-2}
as.vector(scale(x))
```

We can use this as follows to standardize a variable in a tibble:

```{r alternative-use-the-scale-function-3, message=F}
library(tidyverse)
