...
## 5.4.1. What is regression analysis?
...
```{r how-does-it-work-1, echo=F, message=F, fig.height=3,out.width='100%',  warning=F}
...
```{r how-to-use-1, message=F, results='hide'}
library(tidyverse)
library(sjPlot)
...
### 5.4.3.1. Simple linear regression
...
```{r simple-linear-regression-3, eval=F}
...
```{r simple-linear-regression-4, eval=F}
confint(m)
...
```{r simple-linear-regression-5, echo=F, results=F}
...
```{r multiple-regression-1, results=F, message=F}
library(rockchalk)
m_std <- standardize(m)
tab_model(m_std)
```
...
```{r multiple-regression-3, echo=F, results=F}
zm1 = standardize(m1)
zm2 = standardize(m2)
...
For a more detailed explanation of confounding, see the [confounding tutorial](18-linear-regression.qmd).

#### 5.4.3.2.1. Standardized coefficients
...
```{r categorical-ivs-3, echo=F, results=F}
...
#### 5.4.3.3.1. Categories with more than two levels
...
```{r categories-with-more-than-two-levels-1}
...
```{r categories-with-more-than-two-levels-2}
#| html-table-processing: none
...
```{r categories-with-more-than-two-levels-3, echo=F, results=F}
...
```{r categories-with-more-than-two-levels-4}
#| html-table-processing: none
...
```{r interaction-effects-1, out.width="100%", tab.width="100%"}
#| html-table-processing: none
...
```{r interaction-effects-2, echo=F, results=F}
...
## 5.4.4. Conditions and assumptions
...
```{r linearity-1, fig.height=3, results=F, fig.show='hide'}
m <- lm(trust_t1 ~ age, data = d)
...
```{r linearity-2, message=F, warning=F, echo=F, fig.height=3}
library(gridExtra)
set.seed(1)
...
```{r normality-1, fig.height=3, results=F, fig.show='hide'}
...
```{r normality-2, message=F, warning=F, fig.height=3}
#| html-table-processing: none
diag_plots = plot_model(m, type="diag")
diag_plots[[2]]
```
...
#### 5.4.4.1.2. No outliers in the residuals

Outliers can have a large effect on the model.
Usually outliers in the residuals are related to outliers in the variables, which can be detected when [inspecting and cleaning](08-data-cleaning.qmd) the data.
...
```{r no-outliers-in-the-residuals-1, message=F, warning=F, fig.height=3}
diag_plots[[1]]
```


#### 5.4.4.1.3. Homoscedasticity
...
```{r homoscedasticity-1, fig.show=F, message=F, warning=F, fig.height=3}
diag_plots[[3]]
```
...
```{r homoscedasticity-2, echo=F, message=F, warning=F, fig.height=3}
set.seed(1)
...
To check for multicolinearity, you can look at the **correlation matrix** of the independent variables (see the [correlation tutorial](17-correlation.qmd)).
...
If you have multiple variables that are highly correlated, they might be measuring a common factor, so you could consider combining them into a single variable (see the [scale construction](11-scale-construction.qmd) tutorial).
...
### 5.4.5.1. Reporting the coefficients
