...
```{r, echo=F, message=F}
## Here we secretly load the data directly from the folder.
## This is just because we had a quick update and couldn't wait for
## the data on the website to update
d <- read_csv("../../data/practice_data.csv") |>
        mutate(age = if_else(age > 100, 2024 - age, age)) |>
        select(age, political_orientation, political_interest, trust_t1,
               news_consumption = `news consumption`)
```
